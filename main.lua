...
local function startScript2()
    ...
    local indicatorK = createIndicator("KCircle", -25)
    local indicatorESP = createIndicator("ESPCircle", -65)
    ...
    local function getEnemies()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= localPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") then
                ...
                if hum.Health > 0 and not checkIsTeammate(p, hrp) then
    ...
    local function executeKLogic()
        while kEnabled and isRunning do
            ...
            local parts = workspace:GetPartBoundsInRadius(myPos, 200)
            local validParts = {}
            for _, p in ipairs(parts) do
                if p:IsA("BasePart") and p.Size.X >= 10 and p.Size.Z >= 10 and p.CanCollide then
            ...
            for _, enemy in ipairs(enemies) do
                if not kEnabled or not isRunning then break end

                while kEnabled and isRunning and enemy.Character and enemy.Character:FindFirstChild("Humanoid") and enemy.Character.Humanoid.Health > 0 do
                    local enemyHrp = enemy.Character:FindFirstChild("HumanoidRootPart")
                    local enemyHead = enemy.Character:FindFirstChild("Head")
                    if not enemyHrp or not enemyHead then break end

                    local origin = enemyHrp.Position
                    local direction = Vector3.new(0, -100, 0)
                    local params = RaycastParams.new()
                    params.FilterType = Enum.RaycastFilterType.Exclude
                    params.FilterDescendantsInstances = {enemy.Character, char}

                    local result = workspace:Raycast(origin, direction, params)
                    local tpPos

                    if result and result.Instance and result.Instance.Size.X >= 10 and result.Instance.Size.Z >= 10 then
                        tpPos = result.Position - Vector3.new(0, 1.8, 0)
                    else
                        if #validParts > 0 then
                            local fallback = validParts[math.random(1, #validParts)]
                            tpPos = fallback.Position + Vector3.new(0, (fallback.Size.Y/2) - 1.8, 0)
                        else
                            tpPos = origin - Vector3.new(0, 4.5, 0)
                        end
                    end

                    if char:FindFirstChild("HumanoidRootPart") then
                        local lookAt = Vector3.new(enemyHrp.Position.X, tpPos.Y, enemyHrp.Position.Z)
                        char:PivotTo(CFrame.new(tpPos, lookAt))
                    ...
                    local cameraPosition = camera.CFrame.Position
                    local aimPosition = enemyHead.Position + headOffset
                    if usePrediction then
    ...
    local inputBeganConn = UserInputService.InputBegan:Connect(function(input, isProcessed)
        ...
        if input.KeyCode == Enum.KeyCode.K then
            ...
            indicatorK.BackgroundColor3 = kEnabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
            if kEnabled then
                if kThread then task.cancel(kThread) end
                kThread = task.spawn(executeKLogic)
            else
                if kThread then task.cancel(kThread); kThread = nil end
                local char = localPlayer.Character
                if char then
                    for _, part in ipairs(char:GetDescendants()) do
                        if part:IsA("BasePart") then part.CanCollide = true end
        ...
        elseif input.KeyCode == Enum.KeyCode.O then
            ...
            indicatorESP.BackgroundColor3 = ESPEnabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
        end
    end)
